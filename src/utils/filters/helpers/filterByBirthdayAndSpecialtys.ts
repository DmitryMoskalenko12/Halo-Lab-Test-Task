...
export const filterByBirthdayAndSpecialtys = (
  ...
  if (age < 16 && age > 0) {
    ...
    const filterSpec = filteredSpecialtys.filter(spec => doctorsAll.find(doc => doc.specialityId === spec.id && doc.isPediatrician))
    const findOnlySpec = filterSpec.find(spec => spec.name === values.Specialty);
    const filterOnlyDoctors = doctorsAll.filter(doc => findOnlySpec?.id === doc.specialityId);
    const filterCitys = filteredCitys.filter(city => filterOnlyDoctors.find(doc => doc.isPediatrician && city.id === doc.cityId));
